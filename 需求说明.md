...
class CheckpointManager {
    ...
    [void] SaveState([hashtable]$state) {
        $state | ConvertTo-Json -Depth 10 | Out-File $this.CheckpointFile
    }
    
    # 加载执行状态
    [hashtable] LoadState() {
        if (Test-Path $this.CheckpointFile) {
            return Get-Content $this.CheckpointFile | ConvertFrom-Json -AsHashtable
        }
        return @{}
    }
