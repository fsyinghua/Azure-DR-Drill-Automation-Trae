...
try {
    ...
    Write-Host "  检查登录状态失败: $_" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    if ($subscriptions.Count -eq 0) {
        ...
        Write-Host ""
        Write-Host "提示: 请先使用以下命令登录Azure:" -ForegroundColor Yellow
    ...
    Write-Host ""
    Write-Host "提示: 请确保已安装Azure PowerShell模块并已登录Azure" -ForegroundColor Yellow
    exit 1
}

Write-Host ""
Write-Host "步骤 4: 显示详细登录状态..." -ForegroundColor Yellow
try {
    Show-AzureLoginStatus
}
catch {
    Write-Host "  显示登录状态失败: $_" -ForegroundColor Red
}
