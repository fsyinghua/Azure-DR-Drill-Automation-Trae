...
- **SQL 查询优先于循环匹配**: 充分利用 SQL 的 WHERE、JOIN、INDEX 等机制进行数据查询和匹配，避免使用 PowerShell 的 foreach 循环进行线性搜索，大幅提升查询性能
...
- **测试网络配置**（defaults.network，默认 test-dr-vnet/test-dr-subnet）
- **资源发现缓存**（defaults.discovery.cache，默认启用 24 小时 TTL，避免重复扫描订阅）
...
> **设计目标**：用户复制 `AzureConfig.example.json` 后，仅需修改 1-2 项即可运行，其余保持默认即可。

### 1.5 数据库预采集与实时生成方案

#### 1.5.1 设计理念
采用"预采集+实时生成+执行前校验"的三阶段模式，将配置管理从运行时分离到预采集阶段，实现更快的执行速度和更高的可靠性。

#### 1.5.2 三阶段执行流程

**阶段一：预采集（离线/定期执行）**
- 一次性扫描所有订阅、Recovery Services Vault、Replication Protected Items
- 将完整的资源映射关系存储到 SQLite 数据库
- 建立版本控制，支持增量更新
- 生成资源快照，包含所有必要的 ASR 上下文信息

**阶段二：实时生成（切换时执行）**
- 基于数据库中的预采集数据，快速生成准确的切换指令
- 根据用户输入的 VM 名称，从数据库直接获取对应的订阅/Vault/资源组信息
- 生成完整的执行计划，包括所有必要的 PowerShell 命令和参数

**阶段三：执行前校验（运行时验证）**
- 快速验证目标资源的当前状态是否与预采集数据一致
- 检查 Replication Protected Item 的健康状态和复制关系
- 确认目标网络和资源可用性
- 只有校验通过后才执行实际的切换操作
...
CREATE TABLE RecoveryServicesVaults (
    Id TEXT PRIMARY KEY,
    Name TEXT,
    ResourceGroup TEXT,
    SubscriptionId TEXT,
...
CREATE TABLE ReplicationProtectedItems (
    Id TEXT PRIMARY KEY,
    Name TEXT,
    VmName TEXT,
    VaultId TEXT,
...
CREATE TABLE ExecutionPlans (
    Id TEXT PRIMARY KEY,
    PlanName TEXT,
    VmNames TEXT, -- JSON array
    GeneratedAt DATETIME,
    PlanData TEXT, -- JSON: 包含完整的执行指令
    Status TEXT, -- pending, validated, executed, failed
    CreatedBy TEXT
);
```
...
**传统模式**：
- 每次执行：扫描订阅 → 发现 Vault → 获取 Protected Items → 执行
- 耗时：5-10 分钟（取决于订阅数量和网络延迟）

**预采集模式**：
- 预采集：一次性扫描（可定期执行）
- 执行时：数据库查询 → 生成指令 → 校验 → 执行
- 耗时：30-60 秒（主要是校验和实际执行时间）
...
**性能对比**：
- **传统 foreach 方式**：17 台 VM × N 个订阅 × M 个 Vault = O(n×m×k) 复杂度
- **SQL 查询方式**：单次 JOIN 查询 = O(log n) 复杂度（利用索引）
- **预期性能提升**：从秒级降低到毫秒级
    [string]$DatabasePath
